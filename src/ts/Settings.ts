...
export const Settings: ISettings = {
    ...
    init() {
        ...
        if (UserSettings.general.sort.type !== 1)
            (<HTMLInputElement>(
                $id('settings-general-sorting-order')
            )).checked = true;
        else if (UserSettings.general.sort.type === 1)
            (<HTMLInputElement>(
                $id('settings-general-sorting-name')
            )).checked = true;

        (<HTMLInputElement>$id('settings-general-sorting-ascending')).checked =
            UserSettings.general.sort.asc;
        ...
        if (UserSettings.appearance.categories.state === 1) {
            (<HTMLInputElement>(
                $id('settings-appearance-categories-shown')
            )).checked = true;
        } else if (UserSettings.appearance.categories.state === 2) {
            (<HTMLInputElement>(
                $id('settings-appearance-categories-hidden')
            )).checked = true;
            Left.categories.toggle();
        } else {
            (<HTMLInputElement>(
                $id('settings-appearance-categories-remember')
            )).checked = true;
        ...
        // Notes
        (<HTMLInputElement>$id('settings-appearance-notes-showTop')).checked =
            UserSettings.appearance.notes.showTop;
        (<HTMLInputElement>$id('settings-appearance-notes-showText')).checked =
            UserSettings.appearance.notes.showText;
        (<HTMLInputElement>$id('settings-appearance-notes-showTags')).checked =
            UserSettings.appearance.notes.showTags;

        // Top bar
        (<HTMLInputElement>$id('settings-appearance-top-showNotes')).checked =
            UserSettings.appearance.top.addNote;
        (<HTMLInputElement>(
            $id('settings-appearance-top-showCategories')
        )).checked = UserSettings.appearance.top.addCategory;
    ...
    choose(section) {
        ...
        if (this.section !== null) this.unchoose();
    ...
    unchoose() {
        ...
        $id(`settings-main-${sectionToUnset}`).style.transform =
            'translateY(40px)';
    ...
    options: {
        ...
        appearance: {
            categoriesState(state: number) {
                ...
                if (state === 0)
                    UserSettings.appearance.categories.shown =
                        Left.categories.shown;
            },
    ...
    assignListeners() {
        ...
        function handleClick(str: string) {
            if (str === 'settings-general-sorting-order')
                ...
                Settings.options.general.sortingAsc(
                    (<HTMLInputElement>$id(str)).checked
                );
                ...
                Settings.options.appearance.notesTopState(
                    (<HTMLInputElement>$id(str)).checked
                );
            else if (str === 'settings-appearance-notes-showText')
                Settings.options.appearance.notesTextState(
                    (<HTMLInputElement>$id(str)).checked
                );
            else if (str === 'settings-appearance-notes-showTags')
                Settings.options.appearance.notesTagsState(
                    (<HTMLInputElement>$id(str)).checked
                );
            else if (str === 'settings-appearance-top-showNotes')
                Settings.options.appearance.addNoteTopState(
                    (<HTMLInputElement>$id(str)).checked
                );
            else if (str === 'settings-appearance-top-showCategories')
                Settings.options.appearance.addCategoryTopState(
                    (<HTMLInputElement>$id(str)).checked
                );
        ...
        for (let el of els) {
            if ((<HTMLDivElement>el).dataset.for) {
                (<HTMLDivElement>el).onclick = () => {
                    (<HTMLInputElement>(
                        $id((<HTMLDivElement>el).dataset.for)
