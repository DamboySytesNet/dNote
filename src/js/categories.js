    ...
    init(categories) {
        ...
        for (let i = 0; i < categoriesLength; i++) {
            ...
            for (let j = 0; j < notesLength; j++) {
                parsedNotes.push(new Note(categories[i].notes[j].id, categories[i].notes[j].name, categories[i].notes[j].content, categories[i].notes[j].pinned, categories[i].notes[j].protection, categories[i].notes[j].tags, categories[i].notes[j].dateCreated, categories[i].notes[j].dateModified));
            }
